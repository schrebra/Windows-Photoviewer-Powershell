...
# Centralized list of supported extensions
  ...
  <Grid>
      ...
      <Grid x:Name="ImageContainer" Background="#FF0C1018" ClipToBounds="True">
        ...
        <Grid.ContextMenu>
          <ContextMenu>
            ...
            <Separator/>
            <MenuItem x:Name="MenuFit"      Header="Best Fit"/>
            <MenuItem x:Name="MenuActual"   Header="Actual Size (100%)"/>
          </ContextMenu>
        </Grid.ContextMenu>
      </Grid>
    </DockPanel>
    ...
    <Border x:Name="FullScreenOverlay" Visibility="Collapsed"
      ...
      <Grid>
        ...
        <!-- Consolidated Bottom Control Bar -->
        <Border x:Name="FSBottomBar" VerticalAlignment="Bottom" HorizontalAlignment="Center" Margin="0,0,0,24"
                Background="#BB000000" CornerRadius="6" Padding="16,7">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            <!-- Left (Previous) -->
            <Button x:Name="FSPrevBtn" Background="Transparent" BorderThickness="0"
                    Cursor="Hand" Width="38" Height="34" Margin="0,0,6,0">
              <Path Fill="#FFFFFFFF" Data="M 16,4 L 4,18 L 16,32 Z" Stretch="Uniform" Margin="10,6"/>
            </Button>
            ...
            <Button x:Name="FSPauseBtn" Background="Transparent" BorderThickness="0"
              ...
              <Canvas Width="20" Height="20" HorizontalAlignment="Center" VerticalAlignment="Center">
                <Rectangle Width="5" Height="16" Fill="#FFFFFFFF" Canvas.Left="2"  Canvas.Top="2"/>
                <Rectangle Width="5" Height="16" Fill="#FFFFFFFF" Canvas.Left="13" Canvas.Top="2"/>
              </Canvas>
            </Button>
            ...
            <Button x:Name="FSExitBtn" Background="Transparent" BorderThickness="0"
                    Cursor="Hand" Width="38" Height="34" Margin="0,0,6,0">
              <Path Stroke="#FFFFFFFF" StrokeThickness="2.5"
                    StrokeStartLineCap="Round" StrokeEndLineCap="Round"
                    Data="M 4,4 L 16,16 M 16,4 L 4,16" Stretch="Uniform" Margin="10,10"/>
            </Button>

            <!-- Right (Next) -->
            <Button x:Name="FSNextBtn" Background="Transparent" BorderThickness="0"
                    Cursor="Hand" Width="38" Height="34">
              <Path Fill="#FFFFFFFF" Data="M 4,4 L 16,18 L 4,32 Z" Stretch="Uniform" Margin="10,6"/>
            </Button>
          </StackPanel>
        </Border>

        <Border x:Name="FSCountLabelBorder" Background="#88000000" CornerRadius="3"
                HorizontalAlignment="Right" VerticalAlignment="Top" Margin="0,12,18,0" Padding="4,2">
          <TextBlock x:Name="FSCountLabel" Text="" Foreground="#FFFFFFFF"
...
$FSBottomBar        = $window.FindName("FSBottomBar")
$FSCountLabelBorder = $window.FindName("FSCountLabelBorder")
...
$script:imageCache       = New-Object 'System.Collections.Generic.Dictionary[string, byte[]]'
...
$script:oldResizeMode    = 'CanResize'
$script:lastMousePos     = $null

# Fullscreen Idle Timer
$script:fsIdleTimer = New-Object System.Windows.Threading.DispatcherTimer
$script:fsIdleTimer.Interval = [TimeSpan]::FromSeconds(5)
$script:fsIdleTimer.Add_Tick({
    if ($Overlay.Visibility -eq 'Visible') {
        $window.Cursor = [System.Windows.Input.Cursors]::None
        $FSBottomBar.Visibility        = 'Collapsed'
        $FSCountLabelBorder.Visibility = 'Collapsed'
        $script:fsIdleTimer.Stop() # Stop ticking once hidden
    }
})

# Overlay MouseMove (Filters out fake, layout-triggered movements)
$Overlay.Add_MouseMove({
    param($s, $e)
    if ($Overlay.Visibility -ne 'Visible') { return }

    $p = $e.GetPosition($Overlay)
    if ($null -eq $script:lastMousePos) {
        $script:lastMousePos = $p
        return
    }

    $deltaX = [Math]::Abs($p.X - $script:lastMousePos.X)
    $deltaY = [Math]::Abs($p.Y - $script:lastMousePos.Y)

    # Ignore movements smaller than 2px to filter out fake events
    if ($deltaX -gt 2 -or $deltaY -gt 2) {
        $script:lastMousePos = $p

        if ($window.Cursor -eq [System.Windows.Input.Cursors]::None) {
            $window.Cursor = [System.Windows.Input.Cursors]::Arrow
            $FSBottomBar.Visibility        = 'Visible'
            $FSCountLabelBorder.Visibility = 'Visible'
        }

        $script:fsIdleTimer.Stop()
        $script:fsIdleTimer.Start()
    }
})

# Feature 2: Deep Garbage Collection on Minimize
...
# Feature 1: Main Preloader Logic (Fixed delegate scoping)
function Preload-Images {
    if ($script:images.Count -le 1) { return }

    $currentPath = $script:images[$script:currentIndex]
    $prevIndex   = ($script:currentIndex - 1 + $script:images.Count) % $script:images.Count
    $nextIndex   = ($script:currentIndex + 1) % $script:images.Count
    $prevPath    = $script:images[$prevIndex]
    $nextPath    = $script:images[$nextIndex]

    # Purge old cache (Keep memory usage low)
    $keysToRemove = @()
    foreach ($k in $script:imageCache.Keys) {
        if ($k -ne $currentPath -and $k -ne $prevPath -and $k -ne $nextPath) {
            $keysToRemove += $k
        }
    }
    foreach ($k in $keysToRemove) { $script:imageCache.Remove($k) }

    # Define strongly-typed delegate for the WPF Dispatcher to avoid scoping issues
    $preloadAction = [System.Action[string]]{
        param($pathTarget)
        if ($null -ne $script:imageCache -and -not $script:imageCache.ContainsKey($pathTarget)) {
            try {
                $script:imageCache[$pathTarget] = [System.IO.File]::ReadAllBytes($pathTarget)
            } catch {}
        }
    }

    # Queue background task to read adjacent files using BeginInvoke with argument
    foreach ($p in @($nextPath, $prevPath)) {
        if (-not $script:imageCache.ContainsKey($p)) {
...
# Synchronous image loader - Now pulls from RAM cache if available
function Load-Image([string]$path) {
    try {
        $bytes = $null
        if ($script:imageCache.ContainsKey($path)) {
            $bytes = $script:imageCache[$path]
        } else {
...
function Update-Image {
    ...
    # Feature 1: Trigger background precaching for next/prev images
    Preload-Images

    # Feature 2: Silent dynamic Garbage Collection during idle UI states
    $window.Dispatcher.BeginInvoke(
        [System.Windows.Threading.DispatcherPriority]::ContextIdle,
        [System.Action]{ [System.GC]::Collect() }
    ) | Out-Null
}
...
function Open-Images {
    ...
    $script:images.Clear()
    $script:imageCache.Clear()

    if ($fd.FileNames.Count -eq 1) {
        $dir = [System.IO.Path]::GetDirectoryName($fd.FileNames[0])
        $all = [System.IO.Directory]::GetFiles($dir) | Where-Object {
            $script:supportedExts -contains [System.IO.Path]::GetExtension($_).ToLower()
        } | Sort-Object -Unique
...
$MenuFit.Add_Click({ if ($null -ne $MainImage.Source) { Fit-ToWindow } })
$MenuActual.Add_Click({ if ($null -ne $MainImage.Source) { Apply-Zoom 1.0 } })
...
$FitBtn.Add_Click({ if ($null -ne $MainImage.Source) { Fit-ToWindow } })
...
$DeleteBtn.Add_Click({
    ...
    $r = [System.Windows.MessageBox]::Show("Move '$name' to the Recycle Bin?", "Windows Photo Viewer", [System.Windows.MessageBoxButton]::YesNo, [System.Windows.MessageBoxImage]::Question)

    if ($r -eq 'Yes') {
        try {
            [Microsoft.VisualBasic.FileIO.FileSystem]::DeleteFile($path, 'OnlyErrorDialogs', 'SendToRecycleBin')

            $script:imageCache.Remove($path)
            ...
            [System.Windows.MessageBox]::Show("Could not delete.`n$_", "Error", 'OK', 'Error')
        }
    }
})
...
function Enter-Fullscreen {
    ...
    # Strict sequence for true taskbar-hiding fullscreen
    $window.WindowStyle      = 'None'
    $window.ResizeMode       = 'NoResize'
    $window.WindowState      = 'Maximized'

    $script:slideshowRunning = $true
    $script:lastMousePos     = $null # Reset mouse position tracking
    ...
    if ($null -eq $script:slideshowTimer) {
        ...
        $script:slideshowTimer.Add_Tick({ if ($script:slideshowRunning) { Step-Next } })
    }
...
function Exit-Fullscreen {
    ...
    $window.ResizeMode  = $script:oldResizeMode
    ...
    if ($null -ne $script:slideshowTimer) { $script:slideshowTimer.Stop() }
    $script:slideshowRunning = $false
}
...
$FSPauseBtn.Add_Click({
    ...
    if ($script:slideshowRunning) { $script:slideshowTimer.Start() }
    else { $script:slideshowTimer.Stop() }
})
...
$Viewer.Add_PreviewMouseLeftButtonDown({
    ...
    if ($e.ClickCount -eq 2) {
        if ($Overlay.Visibility -eq 'Visible') { Exit-Fullscreen } else { Enter-Fullscreen }
        return
    }
...
$Viewer.Add_PreviewMouseMove({
    ...
    $Viewer.ScrollToHorizontalOffset($script:dragStartHOffset + ($script:dragStartPos.X - $p.X))
    $Viewer.ScrollToVerticalOffset($script:dragStartVOffset + ($script:dragStartPos.Y - $p.Y))
})
...
$window.Add_SizeChanged({
    if ($null -ne $MainImage.Source -and [Math]::Abs($script:currentZoom - $script:fitZoom) -lt 0.002) {
        $script:fitZoom = Get-FitZoom
        Apply-Zoom $script:fitZoom
    }
})
...
$window.Add_KeyDown({
    ...
    switch ($e.Key) {
        'Escape' { if ($Overlay.Visibility -eq 'Visible') { Exit-Fullscreen } }
        'Right'  { Step-Next; $e.Handled = $true }
        'Left'   { Step-Prev; $e.Handled = $true }
        'F11'    { if ($Overlay.Visibility -eq 'Visible') { Exit-Fullscreen } else { Enter-Fullscreen } }
        'Delete' { $DeleteBtn.RaiseEvent([System.Windows.RoutedEventArgs]::new([System.Windows.Controls.Primitives.ButtonBase]::ClickEvent)) }
        'F'      { if ($null -ne $MainImage.Source) { Fit-ToWindow } }
        'Add'    { if ($null -ne $MainImage.Source) { $ZoomInBtn.RaiseEvent([System.Windows.RoutedEventArgs]::new([System.Windows.Controls.Primitives.ButtonBase]::ClickEvent)) } }
        'Subtract' { if ($null -ne $MainImage.Source) { $ZoomOutBtn.RaiseEvent([System.Windows.RoutedEventArgs]::new([System.Windows.Controls.Primitives.ButtonBase]::ClickEvent)) } }
    }

    if ($e.Key -eq 'O' -and ([System.Windows.Input.Keyboard]::Modifiers -band [System.Windows.Input.ModifierKeys]::Control)) {
        Open-Images
        $e.Handled = $true
    }
})
...
$window.Add_Drop({
    ...
    if ($files.Count -eq 1 -and [System.IO.Directory]::Exists($files[0])) {
        $allFolderFiles = [System.IO.Directory]::GetFiles($files[0]) | Where-Object {
            $script:supportedExts -contains [System.IO.Path]::GetExtension($_).ToLower()
        }
        foreach ($f in $allFolderFiles) { $script:images.Add($f) }
    } else {
        foreach ($f in $files) {
            if ($script:supportedExts -contains [System.IO.Path]::GetExtension($f).ToLower()) {
                $script:images.Add($f)
            }
        }
    }
...
if ($cmdArgs.Count -gt 1) {
    ...
    if ([System.IO.File]::Exists($argPath)) {
        ...
        $allCmdFiles = [System.IO.Directory]::GetFiles($dir) | Where-Object {
            $script:supportedExts -contains [System.IO.Path]::GetExtension($_).ToLower()
        }
        foreach ($f in $allCmdFiles) { $script:images.Add($f) }
